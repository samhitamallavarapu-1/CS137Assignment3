...
1) Training path (current train.py): call forward(...) directly with preprocessed tensors.
2) Evaluation-framework path: call adapt_inputs(raw_eval_inputs) then forward(*adapted).
"""
...
class ModelConfig:
    ...
    # Eval/adapt preprocessing settings
    crop_mode: str = "new_england"  # new_england | full | custom
    crop_y0: int = 0
    crop_y1: int = 450
    crop_x0: int = 0
    crop_x1: int = 449
...
class BaselineCNNTransformerPatch(nn.Module):
    def __init__(self, config: ModelConfig) -> None:
        ...
        # Optional normalization buffers for adapt_inputs path.
    ...
    def _calendar_from_hours(self, hours_since_epoch: torch.Tensor) -> torch.Tensor:
        """Build 7D calendar features to mirror train.py.

        Features: [hour_sin, hour_cos, dow_sin, dow_cos, month_sin, month_cos, weekend]
        """
        if hours_since_epoch.ndim != 2:
            raise ValueError("future_time/history_time must be [B, T] int64")

        arr = hours_since_epoch.detach().cpu().numpy().astype(np.int64)
        bsz, steps = arr.shape

        # Vectorized through pandas datetime conversion for month extraction.
    ...
    def _preprocess_raw_weather(self, weather: torch.Tensor) -> torch.Tensor:
        """Convert eval raw weather to model weather input [B,T,C,H,W].

        Accepts either:
        - raw evaluator format [B,T,H,W,C]
        - already-channel-first [B,T,C,H,W]
        """
        if weather.ndim != 5:
        ...
        if weather.shape[-1] == self.config.weather_channels:
            ...
            raise ValueError(
                f"Cannot infer weather channel axis for shape {tuple(weather.shape)} with "
                f"weather_channels={self.config.weather_channels}"
            )
    ...
    def adapt_inputs(
        ...
        """Convert evaluator raw inputs to this model's forward(...) signature.

        Raw evaluator inputs:
            history_weather: [B, Th, 450, 449, C]
            history_energy:  [B, Th, Z]
            future_weather:  [B, Tf, 450, 449, C]
            future_time:     [B, Tf] int64 hours since epoch

        Returns tuple for forward:
            (hist_weather, hist_demand, hist_calendar, fut_weather, fut_calendar)
        """
        if history_energy.ndim != 3 or future_time.ndim != 2:
            raise ValueError("history_energy must be [B,Th,Z] and future_time must be [B,Tf]")

        device = history_weather.device
        bsz, hist_steps, _ = history_energy.shape
        tf = future_time.shape[1]
        ...
        # Derive history hours from first future hour.
        first_future = future_time[:, :1].to(torch.int64)
        offsets = torch.arange(self.config.history_len, 0, -1, device=device, dtype=torch.int64).view(1, -1)
        history_time = first_future - offsets

        if hist_steps != self.config.history_len:
            # Fallback to inferred length if evaluator history differs from config.
            offsets = torch.arange(hist_steps, 0, -1, device=device, dtype=torch.int64).view(1, -1)
            history_time = first_future - offsets
    ...
    def _temporal_encoding(self, total_steps: int, device: torch.device) -> torch.Tensor:
        ...
        return pe.unsqueeze(0).unsqueeze(2)  # [1,T,1,D]
    ...
    def forward(
        ...
        if hist_demand.shape[-1] != self.config.num_zones:
            raise ValueError(
                f"hist_demand last dim must be num_zones={self.config.num_zones}, got {hist_demand.shape[-1]}"
            )
        if hist_calendar.shape[-1] != self.config.calendar_dim or fut_calendar.shape[-1] != self.config.calendar_dim:
            raise ValueError(f"calendar last dim must be calendar_dim={self.config.calendar_dim}")

        if self.config.future_steps > 0 and fut_steps != self.config.future_steps:
            raise ValueError(f"expected Tf={self.config.future_steps} from config, but got Tf={fut_steps}")
...
def get_model(*args, **kwargs) -> BaselineCNNTransformerPatch:
    """Factory supporting both evaluator and training call styles.

    Evaluator style:
        get_model(metadata: dict)

    Training style (current train.py):
